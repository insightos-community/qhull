# qhull: reproducible platform builds
...
Validated musl tag: [`musl-v8.0.2-1`](https://github.com/insightos-community/qhull/releases/tag/musl-v8.0.2-1);
source commit: `7019dc8a86147f874a0a493237a8101ebbae7d87`. Use a normal clone so container packaging can read `.git`.

```bash
git clone https://github.com/insightos-community/qhull.git qhull-repro
cd qhull-repro
git checkout --detach musl-v8.0.2-1
test "$(git rev-parse HEAD)" = 7019dc8a86147f874a0a493237a8101ebbae7d87
```
...
REPRO_WORK="$(mktemp -d "${TMPDIR:-/tmp}/qhull-musl.XXXXXXXX")"
...
- `qhull-8.0.2-musl-x86_64-prefix.tar.gz`
...
The macOS installer obtains Qhull through the locked cmeel wheel dependency set.
...
```bash
gh workflow run musl-release.yml --repo insightos-community/qhull --ref musl-v8.0.2-1
gh run list --repo insightos-community/qhull --workflow musl-release.yml --limit 5
# Set REPRO_RUN_ID to the run ID printed above.
gh run watch "$REPRO_RUN_ID" --repo insightos-community/qhull --exit-status
gh run download "$REPRO_RUN_ID" --repo insightos-community/qhull --name musl-dist --dir downloaded-dist
```
