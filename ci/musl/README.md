# InsightOS qhull 8.0.2 musl release

The `insightos/musl` branch maintains upstream `v8.0.2` with a build/release pipeline. No library source patches are required.

## Assets and runtime

- `qhull-8.0.2-musl-x86_64-prefix.tar.gz`: installed shared library, development headers/metadata, licenses, and manifest.
...
The pipeline compiles shared libraries, runs 11 upstream CTest cases, audits installed ELF files for GLIBC version requirements and unresolved dependencies, and tests the actual release archive after extraction at a new path in a clean, offline musl container. Release publication requires all checks to pass.

Pushes/PRs to `insightos/musl` and manual dispatches upload build artifacts. Push a new tag such as `musl-v8.0.2-1` to publish a GitHub Release. Tags must contain this workflow. Published releases cannot be overwritten by rerunning the workflow; use a new build tag. Upstream tags remain unchanged.
